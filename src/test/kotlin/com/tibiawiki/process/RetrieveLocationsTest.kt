...
class RetrieveLocationsTest {

    private lateinit var target: RetrieveLocations
    ...
    fun setup() {
        ...
        target = RetrieveLocations(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetLocationsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.GEOGRAPHY.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.locationsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetLocationsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.GEOGRAPHY.categoryName)
    ...
    @Test
    fun testGetLocationJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getLocationJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Thais"
        private const val SOME_OTHER_NAME = "Carlin"
        private const val SOME_LIST_NAME = "LocationList"
    }
}
