...
class RetrieveSpellsTest {

    private lateinit var target: RetrieveSpells
    ...
    fun setup() {
        ...
        target = RetrieveSpells(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetSpellsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.SPELL.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.spellsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetSpellsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.SPELL.categoryName)
    ...
    @Test
    fun testGetSpellJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getSpellJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Light"
        private const val SOME_OTHER_NAME = "Find Person"
        private const val SOME_LIST_NAME = "SpellList"
    }
}
