...
class RetrieveCharmsTest {
    ...
    private lateinit var target: RetrieveCharms
    ...
    fun setup() {
        ...
        target = RetrieveCharms(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetCharmsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.CHARM.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.charmsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetCharmsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.CHARM.categoryName)
    ...
    fun testGetCharmJSON() {
        ...
        val result: Optional<JSONObject> = target.getCharmJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Dodge"
        private const val SOME_OTHER_NAME = "Wound"
        private const val SOME_LIST_NAME = "CharmList"
    }
}
