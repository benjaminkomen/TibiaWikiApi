...
class RetrieveStreetsTest {

    private lateinit var target: RetrieveStreets
    ...
    fun setup() {
        ...
        target = RetrieveStreets(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetStreetsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.STREET.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.streetsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetStreetsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.STREET.categoryName)
    ...
    @Test
    fun testGetStreetJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getStreetJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Main Street"
        private const val SOME_OTHER_NAME = "Harbour Street"
        private const val SOME_LIST_NAME = "StreetList"
    }
}
