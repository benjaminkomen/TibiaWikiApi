...
class RetrieveMissilesTest {

    private lateinit var target: RetrieveMissiles
    ...
    fun setup() {
        ...
        target = RetrieveMissiles(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetMissilesJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.MISSILE.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.missilesJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetMissilesJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.MISSILE.categoryName)
    ...
    @Test
    fun testGetMissileJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getMissileJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Arrow"
        private const val SOME_OTHER_NAME = "Bolt"
        private const val SOME_LIST_NAME = "MissileList"
    }
}
