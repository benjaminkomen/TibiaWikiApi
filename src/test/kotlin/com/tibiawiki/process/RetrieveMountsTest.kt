...
class RetrieveMountsTest {

    private lateinit var target: RetrieveMounts
    ...
    fun setup() {
        ...
        target = RetrieveMounts(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetMountsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.MOUNT.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.mountsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetMountsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.MOUNT.categoryName)
    ...
    @Test
    fun testGetMountJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getMountJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Widow Queen"
        private const val SOME_OTHER_NAME = "Titanica"
        private const val SOME_LIST_NAME = "MountList"
    }
}
