...
**P0/P1 issues filed from this review**

| Finding | Issue |
| --- | --- |
| F-01, F-02 | [#446](https://github.com/benjaminkomen/TibiaWikiApi/issues/446) Cloud Build fail-closed on Ready then smoke |
| F-03 | [#447](https://github.com/benjaminkomen/TibiaWikiApi/issues/447) Log uncaught exceptions |
| F-04, F-05 | [#448](https://github.com/benjaminkomen/TibiaWikiApi/issues/448) Docker non-root / exec CMD / `.dockerignore` |
| F-06 | [#449](https://github.com/benjaminkomen/TibiaWikiApi/issues/449) SHA tags, PR-build isolation, leave untagged gcr.io |
| F-07 | [#450](https://github.com/benjaminkomen/TibiaWikiApi/issues/450) Bound wiki-io pool and expand concurrency |
| F-08, F-14 | [#451](https://github.com/benjaminkomen/TibiaWikiApi/issues/451) Fail-closed CI (Gradle action, Sonar gate, pin bun) |
...
1. **Cloud Build fail-closed** ([#446](https://github.com/benjaminkomen/TibiaWikiApi/issues/446), F-01, F-02) — tag `$COMMIT_SHA`, wait Ready, run `smoke:docs` against the **revision URL**. That script hits Swagger + actuator only (no wiki paths, so no Fandom). Align env/probe flags with `deploy.sh`. Drop redundant `gcloud alpha` traffic or document why it exists. Live `https://tibiawiki.dev` smoke stays ops (`deploy.sh` / #444), not PR CI.

2. **Log 500s** ([#447](https://github.com/benjaminkomen/TibiaWikiApi/issues/447), F-03) — five-line production change + test update. Ship immediately after or even before (1).

3. **Docker image hygiene** ([#448](https://github.com/benjaminkomen/TibiaWikiApi/issues/448), F-04, F-05) — non-root, exec CMD, root `.dockerignore`, stop copying tests. Rebuild and confirm Cloud Run still probes.

4. **Registry + PR-build isolation** ([#449](https://github.com/benjaminkomen/TibiaWikiApi/issues/449), F-06) — Artifact Registry, SHA tags, prove `cloudbuild-pr.yaml` cannot clobber prod `:latest`. Memory/concurrency follow-up once expand has metrics (reopen the measurement part of #399, do not guess).

5. **Wiki I/O budget** ([#450](https://github.com/benjaminkomen/TibiaWikiApi/issues/450), F-07) — bounded pool, optional lower expand cap, semaphore. Fixture + Mockito tests; no live Fandom.

6. **CI gates** ([#451](https://github.com/benjaminkomen/TibiaWikiApi/issues/451), F-08, F-14) — `setup-gradle`, pin bun, quality-gate wait, Dependabot limit. Optional OSV. **Do not rename `buid.yml`.**
