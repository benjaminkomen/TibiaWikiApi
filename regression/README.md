...
`/api-docs` is missing `openapi` or is **3.1.x**: the bundled Swagger UI only
accepts `openapi: 3.0.n`, and 3.1.0 produced the tibiawiki.dev error
"The provided definition does not specify a valid version field." It also
rejects a stock Petstore URL in `swagger-initializer.js`.
