...
@Component
class RetrieveQuests(
    articleRepository: ArticleRepository,
    articleFactory: ArticleFactory,
    jsonFactory: JsonFactory
) : RetrieveAny(articleRepository, articleFactory, jsonFactory) {

    val questsList: List<String>
        get() {
            val category = articleRepository.getPageNamesFromCategory(InfoboxTemplate.QUEST.categoryName)
    ...
    val questsJSON: List<WikiJson>
        get() = getArticlesFromInfoboxTemplateAsJSON(questsList)

    fun getQuestJSON(pageName: String): WikiJson? {
