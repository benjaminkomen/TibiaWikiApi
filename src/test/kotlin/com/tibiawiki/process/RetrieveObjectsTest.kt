...
class RetrieveObjectsTest {

    private lateinit var target: RetrieveObjects
    ...
    fun setup() {
        ...
        target = RetrieveObjects(articleRepository, articleFactory, jsonFactory)

        doReturn(SOME_JSON_OBJECT).`when`(jsonFactory).convertInfoboxPartOfArticleToJson("")
    }

    @Test
    fun testGetObjectsJSON_ZeroResults() {
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.OBJECT.categoryName)
        doReturn(emptyList<String>()).`when`(articleRepository).getPageNamesFromCategory(RetrieveAny.CATEGORY_LISTS)

        val result = target.objectsJSON.toList()

        assertThat(result, hasSize(0))
    }

    @Test
    fun testGetObjectsJSON_TwoResults() {
        ...
        doReturn(names).`when`(articleRepository).getPageNamesFromCategory(InfoboxTemplate.OBJECT.categoryName)
    ...
    @Test
    fun testGetObjectJSON() {
        doReturn("").`when`(articleRepository).getArticle(SOME_PAGE_NAME)

        val result: Optional<Map<String, Any>> = target.getObjectJSON(SOME_PAGE_NAME)
    ...
    companion object {
        ...
        private const val SOME_NAME = "Chair"
        private const val SOME_OTHER_NAME = "Table"
        private const val SOME_LIST_NAME = "ObjectList"
    }
}
