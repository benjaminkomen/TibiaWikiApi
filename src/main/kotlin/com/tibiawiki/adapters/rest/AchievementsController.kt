...
class AchievementsController(
    ...
    fun getAchievements(@Parameter(description = "optionally expands the result to retrieve not only " +
            "the achievement names but the full achievements", required = false)
                        @RequestParam(value = "expand", required = false) expand: Boolean?): ResponseEntity<Any> {
        return ResponseEntity.ok()
                .body(if (expand != null && expand) {
    ...
    @ApiResponses(value = [ApiResponse(responseCode = "200", description = "achievement with specified name found"),
    ...
    @ApiResponses(value = [ApiResponse(responseCode = "200", description = "the changed achievement"),
